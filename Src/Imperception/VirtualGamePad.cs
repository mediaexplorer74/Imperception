...
namespace GameManager
{
    class VirtualGamePad
    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            var colorE = Color.Multiply(Color.Yellow, opacity);

            spriteBatch.Draw(texture, 
                new Vector2(64, baseScreenSize.Y - 64), 
                null, color, -MathHelper.PiOver2/*-90o*/, 
                spriteCenter, 1, SpriteEffects.None, 0);

            spriteBatch.Draw(texture, 
                new Vector2(192, baseScreenSize.Y - 64), 
                null, color, MathHelper.PiOver2/*90o*/, 
                spriteCenter, 1, SpriteEffects.None, 0);


            spriteBatch.Draw(texture,
                new Vector2(baseScreenSize.X - 256, baseScreenSize.Y - 128),
                null, colorE, 0, 
                Vector2.Zero, 1, SpriteEffects.FlipVertically, 0);

            spriteBatch.Draw(texture, 
                new Vector2(baseScreenSize.X - 128, baseScreenSize.Y - 128), 
                null, color, 0, 
                Vector2.Zero, 1, SpriteEffects.None/*.FlipVertically*/, 0);
        }
        ...
        public GamePadState GetState(TouchCollection touchState, GamePadState gpState)
        {
            ...
            foreach (var touch in touchState)
            {
                    ...
                    || touch.State == TouchLocationState.Pressed)
                {
                    ...
                    else if (pos.X * Scale < 256)
                        buttonsPressed |= Buttons.DPadRight;
                    else if (pos.X * Scale >= baseScreenSize.X - 256 
                          && pos.X * Scale < baseScreenSize.X - 128)
                        buttonsPressed |= Buttons.B;
                    else if (pos.X * Scale >= baseScreenSize.X - 128)
