...
namespace GameManager
{
    ...
    public class Map1
    {
        public static Point Size { get; } = new Point(8, 12);
        public Tile1[,] Tiles { get; }
        public Point TileSize { get; }

        public Vector2 MapToScreen(int x, int y) => new Vector2(x * TileSize.X, 
            y * TileSize.Y);
        ...
        public Map1()
        {
            ...
            TileSize = new Point(texture.Width, texture.Height);

            for (int y = 0; y < Size.Y; y++)
            {
                for (int x = 0; x < Size.X; x++)
                {
                    Tiles[x, y] = new Tile1(texture, MapToScreen(x, y) 
                        + (TileSize.ToVector2() / 2), x, y);
                }
            }
        }
